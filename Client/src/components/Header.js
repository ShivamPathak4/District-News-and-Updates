...
const Header = ({ isAdmin, setIsAdmin }) => {
  ...
  const navItems = [
    { name: 'All', path: '/' },
    { name: 'Kapilvastu', path: '/Kapilvastu' },
    { name: 'Dumariyaganj', path: '/Dumariyaganj' },
    { name: 'Shorathgarh', path: '/Shorathgarh' },
    { name: 'Bansi', path: '/Bansi' },
    { name: 'Itwa', path: '/Itwa' },
  ];
  ...
  return (
    <header className={`sticky top-0 z-50 ${darkMode ? 'bg-gray-800 text-white' : 'bg-blue-600 text-white'} shadow-md transition-colors duration-300`}>
      <div className="container mx-auto px-4 py-4 flex justify-between items-center">
        <Link to="/" className="text-2xl font-bold">Siddharthnagar News</Link>
